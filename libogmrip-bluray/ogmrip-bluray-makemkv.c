...
{
  guint current, total, max, index, visible, enabled, flags;
  ...
  if (sscanf (buffer, "DRV:%u,%u,%u,%u,\"%s\",\"%s\",\"%s\"", &index, &visible, &enabled, &flags, name, label, device) == 6 && visible > 0)
  {
    DriveData *drive;
...
{
  guint current, total, max, nr, tr, attr, code;
  gchar value[1024];
...
/*
 * copy makemkvcon mkv disc:0 all c:\folder
 */
